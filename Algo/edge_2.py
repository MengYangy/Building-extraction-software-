...
images_path=glob.glob(r'E:\data\model\other_data\tets\images\*.tif')
label_path=glob.glob(r'E:\data\model\other_data\ChainNet\Den_Con\res\pred\*.png')
...
def erode_images_process(erode_img, contours):
    ...
    _, contour, _ = cv.findContours(erode_opt, mode=cv.RETR_EXTERNAL, method=cv.CHAIN_APPROX_NONE)
...
def detction_overlap_building(input_img, input_edge, kernel_size, iteration):
    ...
    data = img.copy()
    _, res1, _ = cv.findContours(data, mode=cv.RETR_EXTERNAL, method=cv.CHAIN_APPROX_NONE)
    target_num = len(res1)
    ...
    _,contours1, _ = cv.findContours(gray_e1, mode=cv.RETR_EXTERNAL, method=cv.CHAIN_APPROX_NONE)
    ...
    _,contours2, _ = cv.findContours(gray_e2, mode=cv.RETR_EXTERNAL, method=cv.CHAIN_APPROX_NONE)
    opt_ero1, contours2, bad_erode2 = erode_images_process(erosion2, contours2)
    if bad_erode2 != []:
        plot_bad_erode(erosion2, bad_erode2)
...
def detection(images_path, label_path, save_path=r"E:\data\model\other_data\tets\save"):
    ...
    re, erode1, erode2, dis, add, k, dis1, add1, k1 = detction_overlap_building(initial_img, contours, 5, 1)
    ...
    if dis1 != None:
        for i in range(len(dis1)):
            cv.rectangle(chang_bbox, (dis1[i][0], dis1[i][1]), (dis1[i][2], dis1[i][3]), (255, 0, 0), 2)
    cv.imwrite(save_path + '/' + img_names + "/{}_change.png".format(img_names), chang_bbox)
    #     原始的框无法对应腐蚀之后的那些框
    #     无法对应有两种可能：1.物体一分为二，导致bbox无法与之对应
    #                         2.小目标经过腐蚀之后面积小于阈值直接被填充了
    ...
    if add is not None:
        for i in range(len(add)):
            cv.rectangle(erode1, (add[i][0], add[i][1]), (add[i][2], add[i][3]), (0, 255, 0), 2)
...
for i in range(len(images_path)):
    detection(images_path[i],label_path[i])